...
class PackingDiscrete(gym.Env):
    def __init__(self,
        ...
        # Define action space: Discrete with leaf_node_holder * 2 actions (2 for rotation: 0 for no rotation, 1 for rotation)
        self.action_space = gym.spaces.Discrete(self.leaf_node_holder * 2)
    ...
    def step(self, action):
        # Decode action
        rotation_flag = action // self.leaf_node_holder  # Determine rotation (0 or 1)
        leaf_node_idx = action % self.leaf_node_holder  # Get the leaf node index

        # Get the position corresponding to the leaf node
        position = self.get_possible_position()[leaf_node_idx]

        # Prepare the action tuple
        action = (rotation_flag, position[0], position[1])

        # Now perform the drop operation
        succeeded = self.space.drop_box(self.next_box, [action[1], action[2]], rotation_flag, self.next_den, self.setting)
        ...
        self.packed.append(
            [packed_box.x, packed_box.y, packed_box.z, packed_box.lx, packed_box.ly, packed_box.lz])
